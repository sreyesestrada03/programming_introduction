...
def calcular_puntaje_jugador(jugador: dict) -> int:
    ...
    if jugador["position"] == "SUB" or jugador["position"] == "RES":
        ...
        if "L" in jugador["position"] and jugador["preferred_foot"] == "Left" or "R" in jugador["position"] and jugador[
...
def existe_jugador_edad(clubes: dict, edad: int) -> bool:
    existe_jugador = False

    for club in clubes:
        i = 0
        while i < len(clubes[club]) and existe_jugador == False:
            if clubes[club][i]["age"] == edad:
                existe_jugador = True

            i += 1

    return existe_jugador


def cuantos_jugadores_por_edad(clubes: dict) -> dict:
    dict_jugadores_por_edad = {}

    for club in clubes:
        for jugador in clubes[club]:
            if jugador["age"] in dict_jugadores_por_edad:
                dict_jugadores_por_edad[jugador["age"]] += 1
            else:
                dict_jugadores_por_edad[jugador["age"]] = 1

    return dict_jugadores_por_edad


def jugador_mas_viejo(clubes: dict) -> dict:
    dict_jugador_mas_viejo = {}
    jugador_mas_viejo_dict = {"age": 0}
    jugador_mas_viejo_nombre = ""

    for club in clubes.keys():
        for cada_jugador in clubes[club]:
            if jugador_mas_viejo_dict["age"] < cada_jugador["age"]:
                jugador_mas_viejo_dict["age"] = cada_jugador["age"]
                jugador_mas_viejo_nombre = cada_jugador["name"]

    dict_jugador_mas_viejo[jugador_mas_viejo_nombre] = jugador_mas_viejo_dict["age"]

    return dict_jugador_mas_viejo


# Prueba
clubes = carga_de_datos("players.csv")
print(existe_jugador_edad(clubes, 29))
print(existe_jugador_edad(clubes, 60))
print(cuantos_jugadores_por_edad(clubes))
print(jugador_mas_viejo(clubes))
